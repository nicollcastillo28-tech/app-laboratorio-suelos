-- Geodelta Lab — "Fecha final real" (cuándo terminó de verdad el proyecto), separada de
-- "Fecha final proyecto" (la fecha planeada/estimada que ya existía). Se agrega la columna y
-- se actualiza commit_new_project para que también la reciba al crear un proyecto.
alter table projects add column if not exists fecha_final_real date;
...
begin
  ...
  insert into projects (
    ...
    fecha_bitacora, fecha_ingreso_muestra,
    cliente, correo_cliente, muestra_tomada_por, direccion_cliente,
    telefono_contacto, nombre_contacto,
    fecha_inicio_proyecto, fecha_final_proyecto, fecha_final_real, fecha_recepcion,
  ...
  values (
    ...
    nullif(project_fields->>'fecha_final_real','')::date,
